...
protected:
   ...
   * @post size()=old.size()+1
   */
  void hook(typename ListNode<T>::Ref n, typename ListNode<T>::Ref pos);
   ...
   * @post size()==old.size()-1
   */
  void unhook(typename ListNode<T>::Ref n);
   ...
   */
  typename ListNode<T>::Ref dummy() const;
...
/**
 * @brief Fold a list to an output stream.
 *
 ...
 * @param[in] list is the list to be folded.
 ...
 * @param[in] list is the list to be filled with the input data.
 * @warning If the list is not empty before the call, the new items will be added after the old ones.
 ...
 */
template <class T>
std::istream &operator>>(std::istream &in, List<T> &list) noexcept(false);
